...
@login_required
def messages_list(request):
    # الحصول على جميع المحادثات للمستخدم الحالي مع آخر رسالة
    ...
    for conv in conversations:
        ...
        last_message = conv.last_messages[0] if conv.last_messages else None
...
@login_required
@csrf_exempt  # إضافة هذا الديكور إذا كانت المشكلة متعلقة بـ CSRF (للاختبار فقط)
def send_message(request, conversation_id):
    if request.method == 'POST' and request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        if not content:
            return JsonResponse({'status': 'empty_content'}, status=400)
...
def get_messages(request, conversation_id):
    ...
    ) if last_message_id else conversation.messages.all()
